...
from p1ch5.model_data import ModelData
...
def polynomial_model(model_data: ModelData, parameters: torch.tensor) -> torch.tensor:
    # m = Polynomial model
    ...
    return (weight_2 * (model_data.temp_unknown ** 2)) + (weight_1 * model_data.temp_unknown) + bias
...
def training_loop(n_epochs: int, optimizer: torch.optim,
    ...
    for epoch in range(n_epochs):
        ...
        # Forward Pass
        training_temperature_prediction = polynomial_model(model_data=training_data, parameters=parameters)
        ...
        if validation_data is not None:
            with torch.no_grad():
                ...
                validation_temperature_prediction = polynomial_model(model_data=validation_data, parameters=parameters)
        ...
        if not print_parameters and epoch % print_step == 0:
            ...
            print(
                f"Epoch: {epoch}, Training loss: {float(training_loss):.4f}, Validation loss: {float(validation_loss):.4f}"
                f"\n\tWeights: {parameters[:-1]}, Bias: {parameters[-1:]}"
